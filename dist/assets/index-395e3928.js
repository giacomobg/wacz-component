 ...
 */class kt extends wt{}kt.directiveName="unsafeSVG",kt.resultType=2;const Et=vt(kt),Ft=vt(class extends Bt{constructor(g){var e;if(super(g),g.type!==Nt||g.name!=="style"||((e=g.strings)===null||e===void 0?void 0:e.length)>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(g){return Object.keys(g).reduce((e,r)=>{const o=g[r];return o==null?e:e+`${r=r.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${o};`},"")}update(g,[e]){const{style:r}=g.element;if(this.ct===void 0){this.ct=new Set;for(const o in e)this.ct.add(o);return this.render(e)}this.ct.forEach(o=>{e[o]==null&&(this.ct.delete(o),o.includes("-")?r.removeProperty(o):r[o]="")});for(const o in e){const s=e[o];s!=null&&(this.ct.add(o),o.includes("-")?r.setProperty(o,s):r[o]=s)}return He}});var rt=__webpack_require__(6274),Ht=__webpack_require__(3029),ut=__webpack_require__.n(Ht);const ti=m(rt.Z);function Ee(g){return[ti,g]}const je=window.IS_APP||window.electron&&window.electron.IS_APP||window.matchMedia("(display-mode: standalone)").matches,It="1.8.2";function me(g){g.key==" "&&(g.preventDefault(),g.target.click())}class Pt extends _e{constructor(){super(),this.size="1.1em",this.width=null,this.height=null}static get properties(){return{svg:{type:String},size:{type:String},width:{type:String},height:{type:String}}}static get styles(){return k`
    ...
    `}}),customElements.define("wr-modal",class extends _e{constructor(){super(),this.title="",this.bgClass="",this.noBgClose=!1}static get properties(){return{title:{type:String},bgClass:{type:String},noBgClose:{type:Boolean}}}static get styles(){return Ee(k`
    .modal-background {
...
${f.toString()}`),r(this.errorMsg)};return Ta(this.scope+this.name,{registrationOptions:{scope:this.scope},registered(){console.log("Service worker is registered"),e()},error(f){s(f)}}),o}getCrossOriginIframeMsg(){return this.isCrossOriginIframe()?`      Sorry, Service Workers can not be used in cross-origin iframes.
  ...
  `)}render(){return $`
    ...
    `}renderCollInfo(e){return $`<wr-coll-info .coll=${e}></wr-coll-info>`}renderEmpty(){return $`<i>No Archives so far! Archives loaded in the section above will appear here.</i>`}}class ri extends _e{constructor(){super(),this.detailed=!1,this.canDelete=!1}static get properties(){return{coll:{type:Object},detailed:{type:Boolean},canDelete:{type:Boolean}}}static get styles(){return Ee(ri.compStyles)}static get compStyles(){return k`
    .columns {
      width: 100%;
    }
    .column {
  ...
  `}renderSummaryView(){const e=this.coll;return $`
    ...
    .embed-globe {
      ...
      border-style: solid;
      line-height: 0.5em;
    }`}render(){let{numValid:e,numInvalid:r,domain:o,certFingerprint:s,datapackageHash:f,publicKey:w,software:z}=this.collInfo.verify||{};e=e||0,r=r||0;const A=this.collInfo.sourceUrl,R=s?`https://crt.sh/?q=${s}`:"",X=Si(this.ts).toLocaleString();return $`
...
`).map(R=>{const X=R.match(/^\s+/);if(X===null)return R;const[te]=X;return te.length>=A.length?R.slice(A.length):R}).join(`
`)}(o,r[3]||"");return{type:"code",raw:o,lang:r[2]?r[2].trim():r[2],text:s}}}heading(e){const r=this.rules.block.heading.exec(e);if(r){let o=r[2].trim();if(/#$/.test(o)){const f=di(o,"#");this.options.pedantic?o=f.trim():f&&!/ $/.test(f)||(o=f.trim())}const s={type:"heading",raw:r[0],depth:r[1].length,text:o,tokens:[]};return this.lexer.inline(s.text,s.tokens),s}}hr(e){const r=this.rules.block.hr.exec(e);if(r)return{type:"hr",raw:r[0]}}blockquote(e){const r=this.rules.block.blockquote.exec(e);if(r){const o=r[0].replace(/^ *> ?/gm,"");return{type:"blockquote",raw:r[0],tokens:this.lexer.blockTokens(o,[]),text:o}}}list(e){let r=this.rules.block.list.exec(e);if(r){let o,s,f,w,z,A,R,X,te,ee,pe,Pe,Ce=r[1].trim();const Ke=Ce.length>1,ge={type:"list",raw:"",ordered:Ke,start:Ke?+Ce.slice(0,-1):"",loose:!1,items:[]};Ce=Ke?`\\d{1,9}\\${Ce.slice(-1)}`:`\\${Ce}`,this.options.pedantic&&(Ce=Ke?Ce:"[*+-]");const Ve=new RegExp(`^( {0,3}${Ce})((?: [^\\n]*)?(?:\\n|$))`);for(;e&&(Pe=!1,r=Ve.exec(e))&&!this.rules.block.hr.test(e);){if(o=r[0],e=e.substring(o.length),X=r[2].split(`
`,1)[0],te=e.split(`
`,1)[0],this.options.pedantic?(w=2,pe=X.trimLeft()):(w=r[2].search(/[^ ]/),w=w>4?1:w,pe=X.slice(w),w+=r[1].length),A=!1,!X&&/^ *$/.test(te)&&(o+=te+`
`,e=e.substring(te.length+1),Pe=!0),!Pe){const Re=new RegExp(`^ {0,${Math.min(3,w-1)}}(?:[*+-]|\\d{1,9}[.)])`);for(;e&&(ee=e.split(`
`,1)[0],X=ee,this.options.pedantic&&(X=X.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),!Re.test(X));){if(X.search(/[^ ]/)>=w||!X.trim())pe+=`
`+X.slice(w);else{if(A)break;pe+=`
`+X}A||X.trim()||(A=!0),o+=ee+`
`,e=e.substring(ee.length+1)}}ge.loose||(R?ge.loose=!0:/\n *\n *$/.test(o)&&(R=!0)),this.options.gfm&&(s=/^\[[ xX]\] /.exec(pe),s&&(f=s[0]!=="[ ] ",pe=pe.replace(/^\[[ xX]\] +/,""))),ge.items.push({type:"list_item",raw:o,task:!!s,checked:f,loose:!1,text:pe}),ge.raw+=o}ge.items[ge.items.length-1].raw=o.trimRight(),ge.items[ge.items.length-1].text=pe.trimRight(),ge.raw=ge.raw.trimRight();const st=ge.items.length;for(z=0;z<st;z++){this.lexer.state.top=!1,ge.items[z].tokens=this.lexer.blockTokens(ge.items[z].text,[]);const Re=ge.items[z].tokens.filter(ze=>ze.type==="space"),lt=Re.every(ze=>{const pt=ze.raw.split("");let _t=0;for(const hi of pt)if(hi===`
`&&(_t+=1),_t>1)return!0;return!1});!ge.loose&&Re.length&&lt&&(ge.loose=!0,ge.items[z].loose=!0)}return ge}}html(e){const r=this.rules.block.html.exec(e);if(r){const o={type:"html",raw:r[0],pre:!this.options.sanitizer&&(r[1]==="pre"||r[1]==="script"||r[1]==="style"),text:r[0]};return this.options.sanitize&&(o.type="paragraph",o.text=this.options.sanitizer?this.options.sanitizer(r[0]):Le(r[0]),o.tokens=[],this.lexer.inline(o.text,o.tokens)),o}}def(e){const r=this.rules.block.def.exec(e);if(r)return r[3]&&(r[3]=r[3].substring(1,r[3].length-1)),{type:"def",tag:r[1].toLowerCase().replace(/\s+/g," "),raw:r[0],href:r[2],title:r[3]}}table(e){const r=this.rules.block.table.exec(e);if(r){const o={type:"table",header:ji(r[1]).map(s=>({text:s})),align:r[2].replace(/^ *|\| *$/g,"").split(/ *\| */),rows:r[3]?r[3].replace(/\n[ \t]*$/,"").split(`
...
`+o.raw,s.text+=`
`+o.text,this.inlineQueue.pop(),this.inlineQueue[this.inlineQueue.length-1].src=s.text):r.push(o);else if(e){const z="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(z);break}throw new Error(z)}}return this.state.top=!0,r}inline(e,r){this.inlineQueue.push({src:e,tokens:r})}inlineTokens(e,r=[]){let o,s,f,w,z,A,R=e;if(this.tokens.links){const X=Object.keys(this.tokens.links);if(X.length>0)for(;(w=this.tokenizer.rules.inline.reflinkSearch.exec(R))!=null;)X.includes(w[0].slice(w[0].lastIndexOf("[")+1,-1))&&(R=R.slice(0,w.index)+"["+Ni("a",w[0].length-2)+"]"+R.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(w=this.tokenizer.rules.inline.blockSkip.exec(R))!=null;)R=R.slice(0,w.index)+"["+Ni("a",w[0].length-2)+"]"+R.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);for(;(w=this.tokenizer.rules.inline.escapedEmSt.exec(R))!=null;)R=R.slice(0,w.index)+"++"+R.slice(this.tokenizer.rules.inline.escapedEmSt.lastIndex);for(;e;)if(z||(A=""),z=!1,!(this.options.extensions&&this.options.extensions.inline&&this.options.extensions.inline.some(X=>!!(o=X.call({lexer:this},e,r))&&(e=e.substring(o.raw.length),r.push(o),!0))))if(o=this.tokenizer.escape(e))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.tag(e))e=e.substring(o.raw.length),s=r[r.length-1],s&&o.type==="text"&&s.type==="text"?(s.raw+=o.raw,s.text+=o.text):r.push(o);else if(o=this.tokenizer.link(e))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.reflink(e,this.tokens.links))e=e.substring(o.raw.length),s=r[r.length-1],s&&o.type==="text"&&s.type==="text"?(s.raw+=o.raw,s.text+=o.text):r.push(o);else if(o=this.tokenizer.emStrong(e,R,A))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.codespan(e))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.br(e))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.del(e))e=e.substring(o.raw.length),r.push(o);else if(o=this.tokenizer.autolink(e,Fi))e=e.substring(o.raw.length),r.push(o);else if(this.state.inLink||!(o=this.tokenizer.url(e,Fi))){if(f=e,this.options.extensions&&this.options.extensions.startInline){let X=1/0;const te=e.slice(1);let ee;this.options.extensions.startInline.forEach(function(pe){ee=pe.call({lexer:this},te),typeof ee=="number"&&ee>=0&&(X=Math.min(X,ee))}),X<1/0&&X>=0&&(f=e.substring(0,X+1))}if(o=this.tokenizer.inlineText(f,Kr))e=e.substring(o.raw.length),o.raw.slice(-1)!=="_"&&(A=o.raw.slice(-1)),z=!0,s=r[r.length-1],s&&s.type==="text"?(s.raw+=o.raw,s.text+=o.text):r.push(o);else if(e){const X="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(X);break}throw new Error(X)}}else e=e.substring(o.raw.length),r.push(o);return r}}class xi{constructor(e){this.options=e||Tt}code(e,r,o){const s=(r||"").match(/\S*/)[0];if(this.options.highlight){const f=this.options.highlight(e,s);f!=null&&f!==e&&(o=!0,e=f)}return e=e.replace(/\n$/,"")+`
`,s?'<pre><code class="'+this.options.langPrefix+Le(s,!0)+'">'+(o?e:Le(e,!0))+`</code></pre>
`:"<pre><code>"+(o?e:Le(e,!0))+`</code></pre>
